...
int main() {
    ...
    // for (int i = 0; i < 10; i++) {
        ...
        myTetra->setPosition(1.5f, 1.0f, 0.0f);
        ...
        myTetra->setRotation(-50.0f, 0.0f, 0.0f);
    ...
    // RigidBody* camCollider = new RigidBody(NULL, new SphereCollider(0.2f));
    // camCollider->bounciness = 0.0f;
    // physicsHandler.Enqueue(camCollider);

    while (renderer.isActive()) {
        renderer.BeginLoop();

        // camCollider->position = renderer.camera->position;
